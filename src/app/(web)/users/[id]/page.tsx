...
const UserDetails = (props: { params: { id: string } }) => {
  ...
  const reviewSubmitHandler = async () => {
    if (!ratingText.trim().length || !ratingValue) {
      return toast.error('Inserisci una valutazione e una recensione');
    }

    if (!roomId) toast.error('Id non fornito');
    ...
    try {
      ...
      console.log(error);
      toast.error('Recensione fallita');
    } finally {
      setRatingText('');
      setRatingValue(null);
      setRoomId(null);
    ...
    <div className='container mx-auto px-2 md:px-4 py10'>
      <div className='grid md:grid-cols-12 gap-10'>
        ...
        <div className='md:col-span-8 lg:col-span-9'>
          <div className='flex items-center'>
            <h5 className='text-2xl font-bold mr-3'>Bentornato, {userData.name}</h5>
          </div>
          ...
          <p className='text-xs py-2 font-medium'>
            Registrato dal {userData._createdAt.split('T')[0]}
          </p>
